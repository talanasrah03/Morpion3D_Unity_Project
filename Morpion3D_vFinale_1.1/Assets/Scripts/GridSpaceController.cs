...
private void OnMouseDown()
{
    ...
    if (player == 1)
    {
        mark = Instantiate(GameManager.Instance.selectedXPrefab, spawnPos, Quaternion.identity);
        state = 1;
    }
    // === [⭕ Joueur 2 : O] ===
    else
    {
        mark = Instantiate(GameManager.Instance.selectedOPrefab, spawnPos, Quaternion.identity);
        state = 2;
    }
